...
import { getProviderMatrix, pickLatestMigration } from "@/lib/health-core";
...
test("getProviderMatrix flips individual providers based on the right env vars", () => {
  const matrix = getProviderMatrix({
    ...
    SERPER_API_KEY: "serper-key-123",
    TAVILY_API_KEY: "tvly-key-456",
    EXA_API_KEY: "exa-key-789",
    TEMPORAL_ADDRESS: "temporal.internal:7233",
    TEMPORAL_BRIDGE_SECRET: "x".repeat(32),
    VOICE_LEGAL_LAUNCH_APPROVED: "true",
  });
  assert.equal(matrix.anthropic, true);
  assert.equal(matrix.openai, true);
  assert.equal(matrix.sentry, true);
  assert.equal(matrix.brave, true);
  assert.equal(matrix.serper, true);
  assert.equal(matrix.tavily, true);
  assert.equal(matrix.exa, true);
  assert.equal(matrix.temporal, true);
  assert.equal(matrix.temporal_voice_bridge, true);
