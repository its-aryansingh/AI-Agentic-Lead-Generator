...
export default async function AnalyticsPage() {
  ...
  const [
    ...
    supabase
      .from("prospects")
      .select("id", { count: "exact", head: true })
      .in("qualification_bucket", ["hot", "warm"]),
    supabase
      .from("prospects")
      .select("id", { count: "exact", head: true })
      .not("handoff_generated_at", "is", null),
    supabase
      .from("crm_syncs")
      .select("id", { count: "exact", head: true })
      .eq("status", "completed"),
  ...
  const analyticsData: AnalyticsData = {
    ...
    emailReplies: emailReplies ?? 0,
    voiceCalls: voiceCalls ?? 0,
    voiceCompleted: voiceCompleted ?? 0,
