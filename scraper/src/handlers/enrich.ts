 ...
 */
/**
 * Phone-shaped run pre-filter. Deliberately loose — it exists to (a) tell
 * the model where to look and (b) let us skip the LLM entirely when a page
 * has no phone-shaped text at all.
 *
 * Matched as one greedy run of digits/separators rather than a fixed
 * group pattern: a `\d{3,5}[\s.-]?\d{4,6}` shape silently truncates
 * "0120-4567890" to "0120-456789", and a half-copied landline is worse
 * than no candidate at all. Length is enforced afterwards on the digits.
 *
 * Authoritative parsing happens in lib/enrichment/validator.ts with
 * libphonenumber-js/max.
 */
const IN_PHONE_RX = /\+?\d[\d\s().-]{6,20}\d/g
...
export async function enrichScrapeHandler(
  ...
  try {
    ...
    const scheme = apex.startsWith("127.0.0.1") || apex.startsWith("localhost") ? "http" : "https"
    const disallowed = await readRobots(page, apex, deadline, scheme)
...
async function extractVisibleText(page: Page): Promise<string> {
  return page.evaluate(BROWSER_EXTRACT_TEXT).catch(() => "")
}

/**
 * Runs INSIDE the page, not in Node.
 *
 * scraper/tsconfig.json deliberately sets `lib: ["ES2022"]` with no "DOM":
 * this is a Node service, and adding DOM globally would let server-side
 * code reference `document` and still compile. So the handful of DOM
 * shapes this function needs are declared locally instead — `declare
 * const` in a module is module-scoped, so `document` stays invisible to
 * every other file.
 */
interface DomNode {
...
function BROWSER_EXTRACT_TEXT(): string {
  ...
  for (const a of Array.from(document.querySelectorAll("a[href]"))) {
    ...
    if (href.startsWith("mailto:") || href.startsWith("tel:")) {
      // Many Indian sites render the phone as an icon and put the only
      // copy of the number in the href — innerText alone misses it.
...
async function installRequestGuard(ctx: BrowserContext, apex: string): Promise<void> {
  // `apex` may carry a :port for a local dev fixture; URL.hostname never
  // does. Compare on the hostname alone or every request is aborted.
  const apexHost = apex.toLowerCase().split(":")[0]
  ...
  await ctx.route("**/*", async (route: Route) => {
    ...
    const sameSite = host === apexHost || host.endsWith(`.${apexHost}`)
...
async function readRobots(
  ...
  try {
    const body = await page.evaluate(
      ...
      [`${scheme}://${apex}/robots.txt`, budget] as const,
    )
