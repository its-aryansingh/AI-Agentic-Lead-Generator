...
/**
 * Local-fixture escape hatch, for `npm run dev` and CI integration tests
 * that crawl a fixture site on 127.0.0.1.
 *
 * This HARD-REFUSES to activate in production, whatever the env says.
 * That is deliberate: an SSRF bypass toggle that a stray Railway variable
 * could flip is worse than no toggle at all, because it looks safe in
 * review. There is no way to reach loopback or RFC1918 from a production
 * build of this service.
 */
export function privateHostsAllowed(): boolean {
  if (process.env.NODE_ENV === "production") return false
  return process.env.SCRAPER_ALLOW_PRIVATE_HOSTS === "1"
}
...
export function normalizeDomain(input: string): string {
  ...
  if (net.isIP(host) !== 0 && !privateHostsAllowed()) {
    throw new SsrfBlockedError("IP literal is not a company domain")
  }

  if (!/^[a-z0-9.-]+$/.test(host) || (!host.includes(".") && !privateHostsAllowed())) {
    throw new SsrfBlockedError(`not a valid hostname: "${host}"`)
  }

  // Dev fixtures live on a port. Preserve it so the crawl can reach them.
  return url.port && privateHostsAllowed() ? `${host}:${url.port}` : host
}
...
export async function assertPublicHostname(hostname: string): Promise<string[]> {
  ...
  if (privateHostsAllowed()) return [host]
...
export function isUrlStructurallyAllowed(rawUrl: string): boolean {
  ...
  if (privateHostsAllowed()) return true
...
export async function assertNavigationAllowed(
  ...
  const apex = allowedApex.toLowerCase().split(":")[0]
