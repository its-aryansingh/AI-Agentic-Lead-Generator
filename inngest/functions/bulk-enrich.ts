...
export type BulkEnrichEvent = {
  ...
  data: {
    ...
    playbook_guidance: string | null;
    context_version: number | null;
    context_snapshot: Record<string, unknown> | null;
    playbook_example_ids: string[];
  };
};
  ...
  async ({ event, step }) => {
    ...
    for (const candidate of candidates) {
      ...
      const result = await step.run(
        `enrich-${candidateIdx}-${candidate.name}-${candidate.company}`,
        async () => {
          const draft = draft_email
            ? await draftForProspect({
                ...
                playbookGuidance: playbook_guidance,
              })
            : null;
          ...
          let confidence: "risky" | "invalid" | "unknown" = guess
            ? "risky"
            : "unknown";
          if (domain && guess) {
    ...
    await step.run("persist-prospects", async () => {
      const rows = enriched.map(
        ({ candidate, draft, domain, email, email_confidence }) => ({
          user_id,
          job_id,
      ...
      if (error) {
        throw new Error(`PERSIST_PROSPECTS_FAILED:${error.message}`);
      }
      return inserted;
    });
