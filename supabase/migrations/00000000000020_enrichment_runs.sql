-- =====================================================================
-- 00000000000020_enrichment_runs.sql
...
-- Users read their own runs. Writes are service-role only (the worker).
drop policy if exists "own enrichment runs" on public.enrichment_runs;
create policy "own enrichment runs" on public.enrichment_runs
...
revoke all on function public.complete_enrichment_run(
  uuid, uuid, uuid, text, text, text, text, jsonb, jsonb,
  int, int, int, int, text, int, int, int, text, text
) from public, anon, authenticated;

grant execute on function public.complete_enrichment_run(
  uuid, uuid, uuid, text, text, text, text, jsonb, jsonb,
  int, int, int, int, text, int, int, int, text, text
) to service_role;
