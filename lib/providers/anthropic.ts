...
async function aiFor(userId: string | undefined, purpose: "writing") {
  if (!userId) {
  ...
  return (await import("../ai-config")).resolveAiModel(userId, purpose);
}
...
export async function getChatModel(userId: string) {
  return (await import("../ai-config")).resolveAiModel(userId, "chat");
}
...
export const DraftSchema = z.object({
  research_summary: z
    .string()
    .describe(
      "2-3 sentence factual summary of who this person is and what their company does. No fluff.",
    ),
  email_subject: z
    .string()
    .max(60)
    .describe(
      "Cold email subject line. <50 chars ideal. No clickbait, no fake re: or fwd:.",
    ),
  email_body: z
    .string()
    .describe(
  ...
  talking_points: z
    ...
    .length(3)
    .describe(
      "Three concrete conversation starters for a follow-up call or LinkedIn DM.",
    ),
});
...
export async function draftForProspect(opts: {
  ...
  const resolved = await aiFor(opts.userId, "writing");
  if (!resolved) {
  ...
  try {
    const { object } = await generateObject({
      ...
      prompt: [
        ...
        customerContext
          ? `Approved seller context (claims may be used only as written):\n${customerContext}`
          : "",
        playbookGuidance ?? "",
...
export const ReplyDraftSchema = z.object({
  subject: z
    .string()
    .max(80)
    .describe("Reply subject. Usually 'Re: <original subject>'."),
  body: z
    .string()
    .describe(
  ...
  next_step: z
    ...
    .describe(
      "The intent of this reply — what we're trying to move the conversation toward.",
    ),
});
...
function mockReplyDraft(opts: {
  ...
  const calendarLink = opts.calendar_url
    ? opts.calendar_url
    : "[calendar link]";
