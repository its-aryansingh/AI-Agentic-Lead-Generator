...
- If a search comes back empty, say so plainly.`,
  ...
  researcher: {
    ...
    emoji: "🧪",
    modelTier: "research",
    // One enrichment call per named prospect plus a final synthesis step.
    // Five-result discovery is the default UI sample size.
    maxSteps: 6,
...
- Use enrich_prospect once per named person in the instruction. You may call it for up to five people.
...
Match the user's register if a voice anchor is provided; otherwise: professional, warm, direct, not corporate. Return the subject + body (and a one-line note on what you changed if you revised an existing draft). You have no tools — this is pure writing.`,
  ...
  compliance: {
    ...
    maxSteps: 1,
    toolNames: [],
    systemPrompt: `You are the Compliance & Ops reviewer on an AI BDR team. You review cold outbound copy for legal and deliverability hygiene.

Note: Mandatory CAN-SPAM physical sender address, suppression list filtering, and one-click unsubscribe links are automatically attached by the system at send time. B2B cold email outreach with legitimate interest is compliant under CAN-SPAM, GDPR B2B, and India DPDP.

Check against: truthful non-deceptive subject lines, no offensive content, no spam-trigger phrases, respectful tone.

Return a short verdict — PASS (with a one-line thumbs-up) or NEEDS-FIX only if genuinely deceptive or high spam risk. Be terse. You have no tools; you reason over what you're given.`,
  },
...
export const SPECIALIST_NAMES = Object.keys(
  SPECIALIST_META,
) as SpecialistName[];
