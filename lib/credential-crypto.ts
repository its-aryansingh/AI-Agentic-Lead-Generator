...
function key(secret: string) {
  if (secret.length < 24)
    throw new Error(
      "Credential encryption secret must be at least 24 characters.",
    );
  return Buffer.from(
    crypto.hkdfSync(
      "sha256",
      Buffer.from(secret),
      Buffer.from("salesengai-credentials"),
...
export function credentialSecret() {
  const secret =
    process.env.CONNECTION_ENCRYPTION_KEY ?? process.env.MAILBOX_STATE_SECRET;
...
export function decryptCredential(payload: string, secret?: string) {
  ...
  if (process.env.CONNECTION_ENCRYPTION_KEY) {
    candidateSecrets.push(process.env.CONNECTION_ENCRYPTION_KEY);
  }
  if (
    process.env.MAILBOX_STATE_SECRET &&
    !candidateSecrets.includes(process.env.MAILBOX_STATE_SECRET)
  ) {
    candidateSecrets.push(process.env.MAILBOX_STATE_SECRET);
  }

  if (candidateSecrets.length === 0) {
    throw new Error("CONNECTION_ENCRYPTION_KEY is not configured.");
  }
